#!/usr/bin/env python3
"""Open the frozen self-configuring POR residue-9 test exactly once."""
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Score the frozen self-configuring Hybrid-POR on locked test.",
...
def main(argv=None):
    ...
    bundle_path = output / "trained_self_configuring_dg_npor_v9.joblib"
    ...
    if not bool(bundle["method_resolved_before_test"]):
        raise RuntimeError("Frozen self-configuring method is unresolved.")
    ...
    with H5StructuredTrackSource(
        h5_file, dataset.row_index, max_tracks=int(bundle["max_tracks"])
    ) as source:
        encoded = encoder.encode_source(source, test, batch_size=args.read_batch_size)
    prediction = predict_batches(
        model, adapter, encoded["geometry"], args.query_batch_size
    )
    ...
    for efficiency in (0.60, 0.70, 0.77, 0.85):
        ...
        row["method"] = "SelfConfig-Hybrid-POR-full-PSD"
        row["score_source"] = "self_configuring_structured_POR_probability"
        rows.append(row)
    pd.DataFrame(rows).to_csv(
        output / "self_configuring_por_locked_test_working_points.csv", index=False
    )

    comparison = []
    for method, score, source_name in (
        (
            "SelfConfig-Hybrid-POR-full-PSD",
            probability,
            "self_configuring_structured_POR_probability",
        ),
        (
    ...
    pd.DataFrame(
        {
            ...
            "self_configuring_por_probability_b": probability,
            "diagonal_only_probability_b": prediction["diagonal_only_probability"],
            "encoder_auxiliary_probability_audit_only": encoded[
    ...
    ).to_csv(output / "self_configuring_locked_test_predictions.csv.gz", index=False)
    save_json(
        status_path,
        {
            "scored": True,
            "selected_architecture": bundle["selected_architecture"],
    ...
    print("Locked-test self-configuring Hybrid-POR scoring completed")
